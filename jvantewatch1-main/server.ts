...
const DB_FILE = path.join(process.cwd(), 'backend_state.json');
...
const localDb = {
  ...
  saveUsers() { this._save(); },
  saveFriendRequests() { this._save(); },
  saveRoomInvites() { this._save(); },
  saveComments() { this._save(); },
  saveDirectMessages() { this._save(); },
  saveGroupChats() { this._save(); }
};
...
export async function loadExternalDb() {
  try {
    if (fs.existsSync(DB_FILE)) {
      ...
      const loaded = JSON.parse(dbContent);
      localDb.users = loaded.users || {};
      localDb.friendRequests = loaded.friendRequests || {};
      localDb.roomInvites = loaded.roomInvites || {};
      localDb.comments = loaded.comments || {};
      localDb.directMessages = loaded.directMessages || {};
      localDb.groupChats = loaded.groupChats || {};
...
async function startServer() {
  ...
  const voiceChannels: Record<string, any> = {};
  ...
  // Direct Messages
  app.get('/api/dms/:uid1/:uid2', (req, res) => {
    const { uid1, uid2 } = req.params;
    const chatId = [uid1, uid2].sort().join('_');
    const messages = localDb.directMessages[chatId] || [];
    res.json(messages);
  });
  ...
  app.post('/api/dms', (req, res) => {
    ...
    if (!localDb.directMessages[chatId]) localDb.directMessages[chatId] = [];
    
    // Add msg
    const id = generateId();
    const message = { id, from, to, text, createdAt: Date.now() };
    localDb.directMessages[chatId].push(message);
    localDb.saveDirectMessages();
    
    // Emit via socket if recipient is online / listening to dm:chatId
    io.to(`dm_${chatId}`).emit('new_dm', message);
  ...
  app.get('/api/groups', (req, res) => {
    const activeGroups = Object.values(localDb.groupChats).map((g: any) => ({
      id: g.id,
      name: g.name,
  ...
  app.post('/api/groups', (req, res) => {
    const { name, description, creatorUid } = req.body;
    const id = generateId();
    const group = { id, name, description, members: [creatorUid], messages: [], createdAt: Date.now() };
    localDb.groupChats[id] = group;
    localDb.saveGroupChats();
  ...
  app.post('/api/groups/:groupId/join', (req, res) => {
    const { uid } = req.body;
    const group = localDb.groupChats[req.params.groupId];
  ...
  app.post('/api/groups/:groupId/messages', (req, res) => {
    ...
    const id = generateId();
    const message = { id, from, text, createdAt: Date.now() };
    group.messages.push(message);
    localDb.saveGroupChats();

    io.to(`group_${group.id}`).emit('new_group_msg', message);
    res.json(message);
  });
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join_social_room', (socialRoomId) => {
        socket.join(socialRoomId);
    });

    socket.on('leave_social_room', (socialRoomId) => {
        socket.leave(socialRoomId);
    });
    ...
    socket.on('voice_channel_join', (channelId, clientUser) => {
        if (currentVoiceChannelId) {
            ...
            if (voiceChannels[currentVoiceChannelId] && voiceChannels[currentVoiceChannelId].users[socket.id]) {
                delete voiceChannels[currentVoiceChannelId].users[socket.id];
                io.to(`voice_channel_${currentVoiceChannelId}`).emit('voice_channel_state', Object.values(voiceChannels[currentVoiceChannelId].users));
            }
            socket.to(`voice_channel_${currentVoiceChannelId}`).emit('voice_user_left', socket.id);
        }
    ...
    socket.on('voice_joined', () => {
      if (currentVoiceChannelId && voiceChannels[currentVoiceChannelId] && voiceChannels[currentVoiceChannelId].users[socket.id]) {
         voiceChannels[currentVoiceChannelId].users[socket.id].isMicOn = true;
         io.to(`voice_channel_${currentVoiceChannelId}`).emit('voice_channel_state', Object.values(voiceChannels[currentVoiceChannelId].users));
         socket.to(`voice_channel_${currentVoiceChannelId}`).emit('voice_user_joined', socket.id);
    ...
    socket.on('voice_left', () => {
      if (currentVoiceChannelId && voiceChannels[currentVoiceChannelId] && voiceChannels[currentVoiceChannelId].users[socket.id]) {
         voiceChannels[currentVoiceChannelId].users[socket.id].isMicOn = false;
         io.to(`voice_channel_${currentVoiceChannelId}`).emit('voice_channel_state', Object.values(voiceChannels[currentVoiceChannelId].users));
         socket.to(`voice_channel_${currentVoiceChannelId}`).emit('voice_user_left', socket.id);
      } else if (currentRoomId) {
