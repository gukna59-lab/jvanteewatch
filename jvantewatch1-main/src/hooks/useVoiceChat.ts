...
export function useVoiceChat(socket: Socket | null, roomId: string, myId: string | undefined) {
  ...
  const setVolume = (socketId: string, volume: number) => {
    if (audioElementsRef.current[socketId]) {
      audioElementsRef.current[socketId].volume = Math.max(0, Math.min(1, volume));
    }
  };
  ...
  return { isMicOn, toggleMic, speakingUsers, setVolume };
}
