...
import { Mic, MicOff, Smile, Users, X, UserPlus, Globe, Lock, BadgeCheck, LogOut, MessageSquareText } from 'lucide-react';
      ...
      <header className="flex items-center justify-between px-3 sm:px-4 lg:px-6 py-2 sm:py-3 bg-bg-card border-b border-border-card shrink-0 overflow-hidden">
        ...
        <div className="flex items-center gap-1 sm:gap-3 shrink-0">
          ...
          <button 
             ...
             className={`w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-full border transition-all shrink-0 ${isMicOn ? 'bg-emerald-500 text-white border-emerald-500/50 hover:bg-emerald-600 shadow-[0_0_15px_rgba(16,185,129,0.5)]' : 'bg-bg-hover text-zinc-400 border-zinc-700 hover:text-white'}`}
          >
          ...
          <div className="hidden lg:flex -space-x-2 mr-2 cursor-pointer relative group" title="Показать участников">
             {users.slice(0, 3).map((u, i) => (
               <div key={u.id} className="relative z-10" style={{ zIndex: 10 - i }}>
                 <div title={u.username} className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-[10px] font-bold text-white shadow-sm shrink-0 relative ${u.isCreator ? 'creator-frame' : speakingUsers.has(u.id) ? 'border-emerald-500 overflow-hidden' : 'border-[#11141A] overflow-hidden'}`} style={!u.isCreator && !u.avatar ? { backgroundColor: u.color } : {}}>
                    <div className="w-full h-full rounded-[inherit] overflow-hidden bg-bg-card z-10 relative">
                      {u.avatar ? <img src={u.avatar} alt="avatar" className="w-full h-full object-cover relative z-10" /> : <div className="w-full h-full flex items-center justify-center relative z-10" style={{ backgroundColor: u.color }}>{u.username.substring(0, 2).toUpperCase()}</div>}
                    </div>
                 </div>
                 {u.isCreator && (
               ...
               {users.map(u => (
                 <div key={u.id} className="flex items-center gap-2 px-1">
                   <div className="relative shrink-0">
                     <div className={`w-6 h-6 rounded-full flex items-center justify-center border relative ${u.isCreator ? 'creator-frame' : speakingUsers.has(u.id) ? 'border-emerald-500 overflow-hidden' : 'border-border-card overflow-hidden'}`} style={!u.isCreator && !u.avatar ? { backgroundColor: u.color } : {}}>
                        <div className="w-full h-full rounded-[inherit] overflow-hidden bg-bg-card z-10 relative">
                           {u.avatar ? <img src={u.avatar} className="w-full h-full object-cover relative z-10" /> : <span className="w-full h-full flex items-center justify-center text-[8px] font-bold text-white relative z-10" style={{backgroundColor: u.color}}>{u.username.substring(0,2).toUpperCase()}</span>}
                        </div>
                     </div>
                     {u.isCreator && (
                   ...
                   <span className={`text-sm font-medium truncate flex items-center gap-1 ${speakingUsers.has(u.id) ? 'text-emerald-400' : 'text-text-main'} ${u.isCreator ? 'creator-bubble mt-0.5 mb-0.5 max-w-[max-content]' : ''}`}>
                     {u.username}
                     {u.isCreator && <BadgeCheck className="w-3 h-3 text-white vip-badge shrink-0" />}
                   </span>
                 </div>
               ))}
            </div>
