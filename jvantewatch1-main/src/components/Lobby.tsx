...
import { PlaySquare, Users, Video, LogOut, Settings, Upload, X, Search, Key, Plus, Lock, Globe, UserPlus, Check, XCircle, MonitorPlay, ChevronRight, User as UserIcon, Moon, Star, Heart, BadgeCheck, Clock, MessageSquareText } from 'lucide-react';
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-bg-main font-sans">
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <h1 className="text-3xl font-black tracking-tighter text-[#3B82F6]">JvanteWatch</h1>
          </div>
  ...
  return (
    <div className="min-h-screen bg-bg-main text-text-main font-sans flex flex-col items-center">
      ...
      <motion.header 
        ...
        className="w-full max-w-4xl px-4 py-6 flex items-center justify-between z-10"
      >
        <button onClick={() => setActiveModal('profile')} className="flex items-center gap-3 hover:bg-bg-card p-2 rounded-2xl transition-colors text-left outline-none">
          <div className="relative">
             <div className={`w-12 h-12 bg-bg-card rounded-2xl flex items-center justify-center border ${user?.isCreator ? 'creator-frame' : 'border-border-card overflow-hidden'}`}>
               <div className="w-full h-full rounded-[inherit] overflow-hidden bg-bg-card z-10 relative">
                 {avatar ? <img src={avatar} alt="avatar" className="w-full h-full object-cover" /> : <Logo />}
               </div>
             </div>
             {user?.isCreator && (
               <div className="absolute -top-2 -left-2 w-6 h-6 bg-amber-500 rounded-full flex items-center justify-center border-2 border-[#0A0C10] shadow-sm z-20 pointer-events-none vip-badge">
                 <BadgeCheck className="w-4 h-4 text-white" />
               </div>
             )}
             {country && (
               <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-[#1F2937] rounded-full flex items-center justify-center text-[12px] border-2 border-[#0A0C10] overflow-hidden leading-none shadow-sm z-20">
                 {generateFlagEmoji(country)}
               </div>
             )}
          </div>
          <div className="flex flex-col ml-1">
            <div className={`flex items-center gap-1 ${user?.isCreator ? 'creator-bubble mt-1 mb-1' : ''}`}>
               <h1 className="text-xl font-bold tracking-tight truncate max-w-[150px] leading-tight">{username}</h1>
               {user?.isCreator && <BadgeCheck className="w-5 h-5 text-white vip-badge shrink-0" />}
            </div>
            <p className="text-xs text-emerald-400 mt-0 flex items-center gap-1">
      ...
      <main className="flex-1 w-full max-w-4xl px-4 flex flex-col gap-6 relative pb-32">
        <div className="flex flex-col gap-4">
          ...
          {displayRooms.length === 0 ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12 bg-bg-card rounded-3xl border border-border-card border-dashed">
             ...
             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {displayRooms.sort((a, b) => {
                ...
                return (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1, duration: 0.3 }} whileHover={{ scale: 1.02 }} key={room.id} className="bg-bg-card border border-border-card hover:border-blue-500/50 rounded-2xl p-5 transition-all relative overflow-hidden group flex flex-col h-full cursor-default">
                    {hasFriend && (
                      ...
                      <div className="flex -space-x-2">
                         {room.users.slice(0, 5).map((u: any, i: number) => (
                           <div key={i} className="relative z-10 hover:scale-110 transition-transform" style={{ zIndex: 10 - i }}>
                             <button onClick={(e) => { e.stopPropagation(); openUserProfile(u.uid); }} title={`${u.username} (${getStablePing(u.uid)} мс)`} className={`w-6 h-6 rounded-full border-2 bg-bg-hover flex items-center justify-center relative ${u.isCreator ? 'creator-frame' : 'border-bg-card overflow-hidden'}`}>
                               <div className="w-full h-full rounded-[inherit] overflow-hidden bg-bg-card z-10 relative">
                                 {u.avatar ? <img src={u.avatar} className="w-full h-full object-cover relative z-10" /> : <span className="text-[8px] font-bold text-text-muted relative z-10">{u.username.substring(0, 2).toUpperCase()}</span>}
                               </div>
                             </button>
                             {u.isCreator && (
                               <div className="absolute -top-1 -left-1 w-3 h-3 bg-amber-500 rounded-full flex items-center justify-center border border-[#11141A] z-20 pointer-events-none vip-badge">
      ...
      {/* Bottom Action Bar */}
      <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2, duration: 0.4 }} className="fixed bottom-6 left-0 right-0 flex justify-center px-4 z-50">
                ...
                <div className="space-y-6 relative">
                  ...
                  <div className={`flex flex-col items-center ${viewingUser.isCreator ? 'mt-16 relative z-10' : ''}`}>
                     <div className="relative mb-4 shadow-xl">
                       <div className={`w-24 h-24 rounded-full bg-bg-hover border-4 flex items-center justify-center relative backdrop-blur-xl ${viewingUser.isCreator ? 'creator-frame' : 'border-border-card overflow-hidden'}`}>
                          <div className="w-full h-full rounded-[inherit] overflow-hidden bg-bg-card z-10 relative">
                             {viewingUser.avatar ? <img src={viewingUser.avatar} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-3xl font-bold text-zinc-500 bg-bg-hover">{viewingUser.username.substring(0,2).toUpperCase()}</div>}
                          </div>
                       </div>
                       {viewingUser.country && (
                          <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-[#1F2937] rounded-full flex items-center justify-center text-[18px] border-4 border-[#11141A] overflow-hidden leading-none shadow-md z-20">
                             {generateFlagEmoji(viewingUser.country)}
                          </div>
                       )}
                       {viewingUser.isCreator && (
                          <div title="Верифицированный VIP-аккаунт" className="absolute -top-2 -left-2 w-8 h-8 bg-amber-500 text-white rounded-full flex items-center justify-center border-4 border-[#11141A] shadow-md z-20 cursor-help vip-badge">
                             <BadgeCheck className="w-5 h-5 text-white" />
                          </div>
                       )}
                    </div>
                    <h2 className={`text-2xl font-bold flex items-center gap-1 ${viewingUser.isCreator ? 'creator-bubble' : ''}`}>
                      {viewingUser.username}
                      {viewingUser.isCreator && <span title="Верифицированный VIP-аккаунт" className="flex items-center"><BadgeCheck className="w-5 h-5 text-white vip-badge" /></span>}
                    </h2>
                     ...
                     {viewingUser.uid === user?.uid ? (
                        ...
                        <div className="flex gap-2">
                           {friends.includes(viewingUser.uid) ? (
                              ...
                              <button onClick={() => { setActiveModal(null); handleSendRequest(viewingUser.uid, viewingUser.username, viewingUser.avatar); }} className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-xl transition-all">В друзья</button>
                           )}
                           <button onClick={() => { setActiveModal(null); window.dispatchEvent(new CustomEvent('open_dm', { detail: viewingUser })); }} className="w-full py-3 bg-zinc-800 hover:bg-zinc-700 text-white font-semibold rounded-xl border border-zinc-700 transition-all">Написать</button>
                        </div>
                     )}
                  </div>
                </div>
                          ...
                          <div key={friend.uid} className="flex items-center justify-between p-3 bg-bg-main rounded-xl border border-border-card">
                            <button onClick={() => openUserProfile(friend.uid)} className="flex items-center gap-3 hover:opacity-80 transition-opacity outline-none text-left flex-1 min-w-0">
                               <div className="relative shrink-0">
                                 <div className={`w-8 h-8 rounded-full bg-bg-hover flex items-center justify-center border relative ${friend.isCreator ? 'creator-frame' : 'border-border-card overflow-hidden'}`}>
                                   <div className="w-full h-full rounded-[inherit] overflow-hidden bg-bg-card z-10 relative">
                                     {friend.avatar ? <img src={friend.avatar} className="w-full h-full object-cover relative z-10" /> : <div className="w-full h-full flex items-center justify-center text-[10px] font-bold relative z-10">{friend.username.substring(0,2).toUpperCase()}</div>}
                                   </div>
                                 </div>
                                 <div className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border border-[#0A0C10] ${isOnline ? 'bg-emerald-500' : 'bg-zinc-500'} z-20`} title={isOnline ? 'Онлайн' : 'Оффлайн'}></div>
                                 {friend.isCreator && (
                                   <div className="absolute -top-1 -left-1 w-3.5 h-3.5 bg-amber-500 rounded-full flex items-center justify-center border border-[#11141A] z-20 pointer-events-none vip-badge">
                                     <BadgeCheck className="w-2.5 h-2.5 text-white" />
                                   </div>
                                 )}
                               </div>
                               <span className={`text-sm font-medium truncate flex flex-1 items-center gap-1 ${friend.isCreator ? 'creator-bubble mt-1 mb-1 max-w-[max-content]' : ''}`}>{friend.username}</span>
                            </button>
                          </div>
                        )})}
                      </div>
                    ...
                    {searchResult && (
                      <div className="flex items-center justify-between p-3 bg-bg-main rounded-xl border border-pink-500/30">
                         <button onClick={() => openUserProfile(searchResult.uid || searchResult.id)} className="flex items-center gap-3 hover:opacity-80 transition-opacity outline-none text-left flex-1 min-w-0">
                             <div className="relative shrink-0">
                               <div className={`w-8 h-8 rounded-full bg-bg-hover flex items-center justify-center relative border ${searchResult.isCreator ? 'creator-frame' : 'border-border-card overflow-hidden'}`}>
                                 <div className="w-full h-full rounded-[inherit] overflow-hidden bg-bg-card z-10 relative">
                                    {searchResult.avatar ? <img src={searchResult.avatar} className="w-full h-full object-cover relative z-10" /> : <div className="w-full h-full flex items-center justify-center text-[10px] font-bold relative z-10">{searchResult.username.substring(0,2).toUpperCase()}</div>}
                                 </div>
                               </div>
                             ...
                             <span className={`text-sm font-medium truncate ${searchResult.isCreator ? 'creator-bubble max-w-[max-content]' : ''}`}>{searchResult.username}</span>
                         </button>
